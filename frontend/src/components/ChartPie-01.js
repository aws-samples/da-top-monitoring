...
const ChartComponent = memo(({ dataset, title, height="350px", width="100%" }) => {
      
            
            
            var labels = JSON.parse(dataset).map(function (obj) {
                        return obj.name;
            });
            
            
            var series = JSON.parse(dataset).map(function (obj) {
                        return obj.value;
            });
